...
class _SupportScreenState extends State<SupportScreen> {
  ...
  bool _loading    = true;
  bool _refreshing = false; // atualização silenciosa em background
  String? _error;
  ...
  Future<void> _load({bool silent = false}) async {
    // Se tiver cache, exibe imediatamente e atualiza em background
    if (_service.cached != null && !silent) {
      setState(() {
        _data    = _service.cached;
        _loading = false;
      });
    ...
    try {
      final position = await _service.getPosition();

      if (position == null) {
        if (mounted) setState(() {
          _error   = 'Não foi possível obter sua localização.';
          _loading = false;
          _refreshing = false;
        });
        return;
      ...
      if (mounted) setState(() {
        _error      = 'Não foi possível carregar as informações.';
        _loading    = false;
        _refreshing = false;
      });
    }
  ...
  String _feeLabel() {
    ...
    final value = double.tryParse(
        _data!['fee_value']?.toString() ?? '0') ?? 0.0;
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Mensagem de erro não-bloqueante (se houver)
                    if (_error != null && _data != null) ...[
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          ...
                          const Expanded(
                            child: Text(
                              'Exibindo informações da última visita.',
                              style: TextStyle(
...
// ── Widgets locais — idênticos ao original ────────────────────────
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.location_off, size: 64, color: AppTheme.gray),
            const SizedBox(height: 16),
            Text(message,
